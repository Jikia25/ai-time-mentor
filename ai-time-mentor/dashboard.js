...
function renderEmotionCard(profile) {
  ...
  const stressColor = profile.stress < 0.3 ? '#44bd32' : profile.stress < 0.6 ? '#f39c12' : '#e74c3c';
  const focusColor = profile.focusPct > 70 ? '#44bd32' : profile.focusPct > 40 ? '#f39c12' : '#e74c3c';
    ...
    <section class="emotion-card">
      ...
      <div class="metrics-grid">
        <div class="metric-box">
          ...
          <div class="metric-value" style="color: ${focusColor}">${profile.focusPct}%</div>
          <div class="metric-bar">
            <div class="metric-fill" style="width: ${profile.focusPct}%; background: ${focusColor}"></div>
          </div>
        </div>

        <div class="metric-box">
          <div class="metric-label">Stress Level</div>
          <div class="metric-value" style="color: ${stressColor}">${Math.round(profile.stress * 100)}%</div>
          <div class="metric-bar">
            <div class="metric-fill" style="width: ${Math.round(profile.stress * 100)}%; background: ${stressColor}"></div>
          </div>
        </div>

        <div class="metric-box">
          <div class="metric-label">Mood</div>
          <div class="metric-value mood-badge">${getMoodEmoji(profile.mood)} ${profile.mood}</div>
        </div>
      ...
      <div class="detailed-stats">
        <p><small>Typing Intensity: ${profile.typingIntensity} keys/min | Idle Ratio: ${Math.round(profile.idleRatio * 100)}%</small></p>
      </div>
    </section>
  `;
...
function renderUsageStats(usage) {
  ...
  if (!usage || Object.keys(usage).length === 0) {
    container.innerHTML += '<p class="empty">No usage data yet. Start browsing to track activity.</p>';
    return;
  }
  ...
  if (hasCategories) {
    ...
    if (usage.productive > 0) entries.push(['✅ Productive Time', usage.productive, '#44bd32']);
    if (usage.distracting > 0) entries.push(['⚠️ Distracting Time', usage.distracting, '#e74c3c']);
    if (usage.other > 0) entries.push(['📄 Other Time', usage.other, '#95a5a6']);
    total = (usage.productive || 0) + (usage.distracting || 0) + (usage.other || 0) || 1;
  } else {
